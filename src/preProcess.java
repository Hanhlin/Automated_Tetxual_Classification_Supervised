...
//        System.out.println("\n");
    ...
    public static void knn(int k, double[][] tf_idf,double[][] test_tf_idf, int[] clusterAssignments, String[] trainCluster) {
    	
    	System.out.println("**************************** KNN ****************************\n");
    	SimilarityMeasure sm = new SimilarityMeasure("euclidean");
    	
        for (int i = 0; i < test_tf_idf.length; i++) {
        	
            Map<Integer,Double> scores = new HashMap<Integer, Double>(tf_idf.length);
            for (int j = 0; j < tf_idf.length; j++)
                scores.put(j, sm.score(test_tf_idf[i], tf_idf[j]));
            ...
            int cluster = -1, max = Integer.MIN_VALUE;
            int[] count = new int[3];
            for (int j = 0; j < k; j++) {
            	int docIdx = centroidScores.get(j).getKey();
            	count[clusterAssignments[docIdx]]++;
            	if (count[clusterAssignments[docIdx]] > max) {
            		max = count[clusterAssignments[docIdx]];
            		cluster = clusterAssignments[docIdx];
            	}
            }
            
            System.out.println("Document " + (i + 1) + " closest cluster is label " + cluster 
            																+ " ---> " + trainCluster[cluster] + "\n");
            System.out.println(centroidScores.subList(0, k) + "\n");
        }
    }
    
    public static void fKnn(int k, int m, double[][] tf_idf,double[][] test_tf_idf, int[] clusterAssignments, String[] trainCluster) {
    	
    	System.out.println("**************************** Fuzzy KNN ****************************\n");
        ...
        for (int i = 0; i < test_tf_idf.length; i++) {
            ...
            // Create a list from elements of HashMap
            List<Map.Entry<Integer, Double> > opa = new LinkedList<>(scores.entrySet());

            // Sort the list
            Collections.sort(opa, new Comparator<>() {
                public int compare(Map.Entry<Integer, Double> o1, Map.Entry<Integer, Double> o2)
                { return (o1.getValue()).compareTo(o2.getValue()); }
            });
            
            double zero = 0, one = 0, two = 0, total = 0;
            int pow = (2 / (m - 1));
            for (Entry<Integer, Double> entry : opa.subList(0, k)) {
    		    int index = entry.getKey();
    		    int c = clusterAssignments[index];
    		    double score = entry.getValue();
    		    double value = 1 / (Math.pow(score, pow));
    		    total += value;
    		    if (c == 0) {
    		    	zero += value;
    		    }
            ...
            double percent_0 = (zero / total) * 100;
            double percent_1 = (one / total) * 100;
            double percent_2 = (two / total) * 100;
            
            System.out.println("Document " + (i + 1) + " is " + df.format(percent_0) + "% about " + trainCluster[0] +
	    												", " + df.format(percent_1) + "% about " + trainCluster[1] +
	    												", and " + df.format(percent_2) + "% about " + trainCluster[2]);
